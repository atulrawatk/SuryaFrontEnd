...
class OtpController extends GetxController {
  ...
  String otpStatusCheck(){
    return otpStatus==0? AppStrings.confirmBold
        :otpStatus==1?AppStrings.loading:AppStrings.success;
  }

  Color otpButtonCheck(){
    return otpStatus==0? Get.theme.primaryColor
        :otpStatus==1?AppColors.grey_500:AppColors.green;
  }
   ...
   otpVerify() async{
    ...
    if(otpController.text.length<4){
      Get.snackbar(AppStrings.otp, AppStrings.enterValidOTP,snackPosition: SnackPosition.BOTTOM);
      return;
    }
    else{
      setOtpStatus=1;
      ...
      await _apiHelper.otpVerify(body: body).then((res) {
        if(res.isOk){
          ...
          SessionManager().saveToken(otpModel.success.data.token);
          setOtpStatus=2;
  ...
  resendOtp() async{
      ...
      await _apiHelper.resendOtp(body: body).then((res) {
        if(res.isOk){
          ...
          Get.snackbar(AppStrings.mobileVerification, AppStrings.somethingWentWrong);
