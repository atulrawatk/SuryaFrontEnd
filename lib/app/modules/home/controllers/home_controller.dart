...
//Get User Details from DB
  Future getUserDetails() async{
    ...
    if(controller.userModel.value.success!=null){
      ...
      Logger().w("Token =====>>>> ${otpVerifyModel!.success!.data!.verificationToken}");
      Logger().i("=>>>Successfully User Model Imported from DB");
    }
    else{
      setOTPVerifyModel= OtpVerify.fromJson(AppGetStorage.getValue(GetStorageKeys.otpVerifyModelKey));
      Logger().w("Token =====>>>> ${otpVerifyModel!.success!.data!.verificationToken}");
      Logger().i("=>>>Successfully User Model Imported from DB");
    }
  }
...
//Get User Threads From API
  getUserThreads() async{
    ...
    debugPrint(userId);
    await _apiHelper.getUserThreads(userId:otpVerifyModel!.success!.data!.id ,
        authToken:otpVerifyModel!.success!.data!.verificationToken).then((value) {
          Logger().i("=====>>>>> Successful to get User threads");
    }).catchError((error){
      Logger().e("======>>>> Error in Getting User Threads ${error.toString()}");
    });
  }
